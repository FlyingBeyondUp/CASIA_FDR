...
good_observation=generateData(prior=StandardNormal(), signal_ratio=0.5,noise_sigma=1,n_data=12000)
z_good=np.copy(good_observation)
p_good=1-norm.cdf(z_good,loc=0,scale=1)

poor_observation=generateData(prior=StandardNormal(), signal_ratio=0.5,noise_sigma=10,n_data=12000)
z_poor=poor_observation/10
p_poor=1-norm.cdf(z_poor,loc=0,scale=1)

combined_observation=np.array(list(good_observation)+list(poor_observation))
combined_z=np.array(list(z_good)+list(z_poor))
list_s_combined=np.array(list(np.ones(len(good_observation)))+list(10*np.ones(len(poor_observation))))
p_combined=1-norm.cdf(combined_z, loc=0, scale=1)

num_subfigures = 3
fig, axes = plt.subplots(1, num_subfigures, figsize=(2*num_subfigures, 2))
# Plot each subfigure
list_p=[p_good,p_poor,p_combined]
list_p_titles=['Good-precision observations','Poor-precision','Combined']
bins = np.arange(0, 1, 0.05)
for i in range(num_subfigures):
    axes[i].hist(list_p[i],bins,density=True,edgecolor='black')
    axes[i].set_title(list_p_titles[i])
    axes[i].set_aspect(aspect='auto')
    axes[i].set_ylim(0,2)
for ax in axes[1:]:
    ax.set_yticklabels([])
# Adjust layout to prevent overlap
plt.tight_layout()
fig.text(0.5, 0.001, r'p', ha='center')
fig.text(0.004, 0.5, r'density', va='center', rotation='vertical')
plt.savefig('p values of different precision observations.pdf', bbox_inches='tight')


newDeal_good,newDeal_combined=NewDeal(good_observation,np.ones(len(good_observation))),NewDeal(combined_observation,list_s_combined)
newDeal_good.fit()
newDeal_combined.fit()

marginal_p,null_component,alternative_component,lfdr_poor,lfsr_poor=newDeal_good.solve(poor_observation,10*np.ones(len(poor_observation)))
marginal_p,null_component,alternative_component,lfdr_good,lfsr_good=newDeal_good.solve(good_observation,np.ones(len(good_observation)))

plt.scatter(p_poor[p_poor<0.025],np.array(lfsr_poor)[p_poor<0.025],label='poor')
plt.scatter(p_good[p_good<0.025],np.array(lfsr_good)[p_good<0.025],label='good')
plt.legend()

# marginal_p,null_component,alternative_component,lfdr_good,lfsr_good=newDeal_good.solve(list_beta,list_s)
# marginal_p,null_component,alternative_component,lfdr_combined,lfsr_combined=newDeal_combined.solve(list_beta,list_s)

# plt.scatter(lfsr_good,lfsr_combined,color='black')
# plt.scatter(lfdr_good,lfdr_combined,color='blue')
# plt.plot(lfsr_good,lfsr_good,color='red')

# plt.plot(list_beta,lfsr_good)
# plt.plot(list_beta,lfsr_combined)

# plt.scatter(range(0,len(newDeal_good.list_pi)),newDeal_good.list_pi)
# plt.scatter(range(0,len(newDeal_combined.list_pi)),newDeal_combined.list_pi)

# plt.plot(list_beta,lfdr_good,label='lfdr')
# plt.plot(list_beta,lfsr_good,label='lfsr')
# plt.legend()
